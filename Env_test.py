...
class my_env:
    ...
    def reset(self, init_list=None):
        ...
        if init_list is None:
            ...
            while True:
                ...
                pos_i = [dis_obs * np.cos(ang) + cour_random * np.sin(ang),
                         initial_s[1] + np.random.uniform(-150, 150),
                         dis_obs * np.sin(ang) + cour_random * np.cos(ang)]
                if distance.euclidean(initial_s[0:3], pos_i) > self.ss_r:
                    break
        else:
            initial_s = init_list[4:]
            speed_obs = init_list[3]
            ang = init_list[0]
            cour_random = init_list[1]
            alt_random = init_list[2]
            dis_obs = initial_s[2] * speed_obs / self.cruise_as
            pos_i = [dis_obs * np.cos(ang) + cour_random * np.sin(ang),
                     initial_s[1] + alt_random,
                     dis_obs * np.sin(ang) + cour_random * np.cos(ang)]
        vel_i = [-speed_obs * np.cos(ang), 0, -speed_obs * np.sin(ang)]
        ...
        vel = self.as2vel(initial_s[6], initial_s[3], initial_s[4], initial_s[5])  # vx,vy,vz
        self.client.sendDREFs(['sim/time/zulu_time_sec'], [36000, ])  #
        self.client.sendCOMM('sim/operation/reset_flight')  # reset plane
        self.client.sendCOMM('sim/view/circle')  # set the external view
        self.client.sendPOSI(pos)  # 位置
        ...
        rdata = self.get_rdata() + pos_i + vel_i  # 获取实际状态
    ...
    def step(self, action, rdata, tau=1.):
        ...
        target = [target_fp, target_course_ang, 0,
                  target_AS]  # roll，flight path angle，course angle，sideslip angle，airspeed.
