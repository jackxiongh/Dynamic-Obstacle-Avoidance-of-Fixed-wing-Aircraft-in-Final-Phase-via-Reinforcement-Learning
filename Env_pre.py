...
class my_env:
    def __init__(self, name, clientAddr='0.0.0.0', xpHost='127.0.0.1', xpPort=49009, clientPort=1, timeout=10000):
        ...
        self.initial_space = spaces.Box(low=np.array([-800, 1500, 5000, 0, -20, -20, 40, 0, 0, 0]),
                                        high=np.array([800, 2500, 5000, 0, 20, 20, 60, 0, 0, 0]), dtype=np.float32)
    ...
    def get_state(self, rdata, norm=True, clip=True):
        """
        rdata：the raw data from X-plane, represented in the world frame of the X-plane software.
        ...
        state = [rdata[0], -rdata[2], rdata[1]-self.tar_alt] + rdata[3:5] + [d_yaw] + [rdata[7], -rdata[9], rdata[8]] + \
              [pos_r[0], -pos_r[2], pos_r[1]] + [vel_r[0], -vel_r[2], vel_r[1]]
        state[9:] = [0, 0, 0, 0, 0, 0]
        state[1] = 0
        if norm:  # state normalization
            state = self.s_norm(state, self.state_space, clip=clip)
    ...
    def reset(self):
        ...
        initial_s = self.space_sample(self.initial_space)
        speed_obs = np.random.uniform(0, 80)
        while True:
            ang = np.random.uniform(-np.pi, np.pi)
            dis_obs = initial_s[2]*speed_obs/initial_s[6]
            pos_i = [dis_obs * np.cos(ang), initial_s[1], dis_obs * np.sin(ang)]   # position of obstacle
        ...
        init_yaw = initial_s[5] + 360 if initial_s[5] < 0 else initial_s[5]   # [-180,180] to [0, 360]
    ...
    def step(self, action, rdata, tau=1.):
        ...
        next_state = self.get_state(next_rdata)
    ...
    def coor_trans(self, coor):
        ...
        R = 6371393  # The average radius of the Earth
