...
class xplanePID:
    def __init__(self, target, kp, ki, kd):
        self.el = PID_posi(kp[1], ki[1], kd[1], target[0])  # control pitch
        self.tar_roll = PID_posi(kp[2], ki[2], kd[2], target[1])  # control yaw
    ...
    def cal_values(self, state, target=None, roll_max=20.):
        ...
        if target is None:
            target = copy.deepcopy(self.target)
        elevator = self.el.increase(state[1], target=target[0], angle=True)  # pitch
        target_roll = self.tar_roll.increase(state[2], target=target[1], angle=True)   # yaw
...
def course(vx, vy, vz):
    ...
    course angle
    '''
    angle = np.arctan(-vx/vz)*180/np.pi
