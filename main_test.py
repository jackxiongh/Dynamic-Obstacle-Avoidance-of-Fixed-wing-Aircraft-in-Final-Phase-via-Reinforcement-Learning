...
from toolbox.RL_Navi2 import RL_navi
import argparse
from Env_test import my_env
...
parser.add_argument('--name', type=str, default='test8_6a3', help='agent name')
...
def main():
    ...
    navigator = RL_navi(model, model_pre, R_pz=R, ss_r=env.ss_r)
    done, current_steps = False, 0
    for t in range(total_steps):
        t0 = time.time()
        current_steps += 1
        '''Interact & trian'''
        action, dis = navigator.cal_action(rdata, state, env.action_space) #a∈[-1,1]
